    ...
    <section className="relative min-h-[80vh] md:min-h-[100vh] lg:min-h-[110vh] flex flex-col justify-center overflow-hidden text-white !py-20 px-4 sm:px-6 ">
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto flex flex-col justify-between">
        <AnimatePresence mode="wait">
          ...
          >
            <h1 className="text-[30px] md:text-[62px] font-[700] text-center md:text-start max-w-2xl">
              {currentEvent.title}
            </h1>
            <p className="text-[16px] md:text-[20px] text-center md:text-start text-white ">{currentEvent.subtext}</p>
            <p className="text-[16px] md:text-[20px] text-center md:text-start text-white ">{currentEvent.priceText}</p>
            ...
            <div className="flex max-w-lg bg-[#323232] border border-white/10 rounded-sm overflow-hidden backdrop-blur-md group focus-within:border-[#007bff] transition-all">
              ...
              />
              <button className="bg-[#006DF2] px-4 md:px-10 flex items-center gap-2 hover:bg-blue-600 transition-all shrink-0 font-medium uppercase text-[14px] tracking-widest text-white cursor-pointer">
                <Search size={20} /> <span className="hidden md:block">Search</span> 
              </button>
            </div>
