...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 z-100 w-full transition-all duration-300 px-4 sm:px-6 ${
      isScrolled 
        ? ' bg-black/90 ' 
        : ' bg-gradient-to-b from-[#0A0A0CF2] to-[#0A0A0C00]'
    } `}>
      <div className="max-w-7xl mx-auto ">
        <div className="flex justify-between items-center h-16 sm:h-20">
          {/* Center Navigation (Matches Image) */}
            {/* Logo Section */}
          <Link href="/" className="lg:hidden flex items-center flex-shrink-0 group">
            <Image
              src="/travel-logo.png"
              alt="TravelMaster Logo"
              width={120}
          ...
          <div className="hidden lg:flex items-center gap-6 lg:gap-15">
            {/* Logo Section */}
          <Link href="/" className="flex items-center flex-shrink-0 group">
            <Image
              src="/travel-logo.png"
          ...
          <div className='flex items-center gap-6 lg:gap-8'>
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-white text-[12px] lg:text-[15px] font-medium hover:text-white/70 transition-all duration-200 whitespace-nowrap flex items-center"
              >
                {link.label}
                {link.label === 'More' && <span className="ml-1 text-[10px]">▼</span>}
              </Link>
            ))}
          ...
          {/* Right Side Buttons (Matches Image) */}
          <div className="hidden lg:flex items-center gap-4">
            <Link 
              href="#" 
              className="text-white bg-white/20 px-3 py-2 hover:bg-white/30 text-[12px] lg:text-[14px] font-medium transition-colors rounded-full"
            >
              Login/Signup
            </Link>
            <Link 
              href="#" 
              className="bg-white text-black hover:bg-gray-200 rounded-[6px] px-4 lg:px-5 py-2 text-[12px] lg:text-[14px] font-medium transition-all duration-300 whitespace-nowrap"
            >
              Download App
            </Link>
          </div>
        ...
        {isOpen && (
          <>
            ...
            <div className="lg:hidden absolute left-0 right-0 top-full bg-black/95 backdrop-blur-xl border-b border-white/10 animate-in slide-in-from-top-4">
              <div className="flex flex-col px-6 py-6 gap-2">
                ...
                <Link 
                  href="#" 
                  className="mt-4 text-white text-center py-3 text-sm font-bold border border-white/20 rounded-lg"
                  onClick={() => setIsOpen(false)}
                >
                  Login/Signup
                </Link>
                <Link 
                  href="#" 
                  className="bg-white text-black text-center py-3.5 text-sm font-bold uppercase tracking-widest transition-all duration-300 rounded-lg"
                  onClick={() => setIsOpen(false)}
                >
                  Download App
                </Link>
